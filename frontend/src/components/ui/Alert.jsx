    ...
    >
      <div className="flex items-start">
        ...
        <div className="ml-3 flex-1">
          {title && <h3 className={`text-sm font-medium ${typeConfig.text}`}>{title}</h3>}
          <div className={`text-sm ${title ? 'mt-1' : ''} ${typeConfig.text}`}>
            {children}
          </div>
        </div>
        {onClose && (
          <button
            onClick={onClose}
            className={`ml-auto pl-3 -mr-1 ${typeConfig.text} hover:opacity-75 inline-flex`}
          >
